    ...
    <div class="flow-editor">
      <div class="flow-tools">
        ...
        >
          <div class="tool-item" @click="handleUndo">
            <span
              class="icon-undo"
              :class="{ 'icon-undo-disabled': !canUndo }"
            ></span>
          </div>
        </el-tooltip>
        <el-tooltip
      ...
      >
        <div
          ...
          ref="campaignCanvas"
        >
          <div ref="campaignCanvas">
            <div v-for="(flowItem, index) in flowList" :key="flowItem.uuid">
              <div
                class="node-content-wrap"
                @mousedown="handleFlowMoveDown"
                @mouseup="
                  (event) => {
                    handleChangeFlowPosition(flowItem, event);
                  }
                "
                :id="flowItem.uuid"
                @click.stop="
                ...
                "
                :style="{
                  left: flowItem.left + 'px',
                  top: flowItem.top + 'px',
                }"
              >
                <div class="node-content">
                  <draggable
                    class="node-temp-small"
                    :ref="flowItem.uuid"
                    :group="{ name: 'sortable', pull: false, put: true }"
                  >
                  </draggable>
                  <template
                    v-if="flowItem.groupType === flowTypeConstant.action"
                  >
                    ...
                    <div class="flow-text">
                      <div class="text-line">
                        <span>{{
                          flowItem.formData && flowItem.formData.stepName
                        }}</span>
                        <i
                          class="el-icon-close flow-delete"
                          @click.stop="handleDeleteFlowItem(flowItem)"
                        ></i>
                      </div>
                    </div>
                    <!--    -->
                  ...
                  <template
                    v-else-if="
                      flowItem.groupType === flowTypeConstant.condition
                    "
                  >
                    ...
                    <div class="flow-text">
                      ...
                      <div class="text-line1">
                        <template
                          v-if="
                            flowItem.formData && flowItem.formData.ifNodeTitle
                          "
                        >
                          <span class="text-content">{{
                            flowItem.formData.ifNodeTitle
                          }}</span>
                        </template>
                      </div>
                    </div>
                  ...
                  <!-- flow -->
                  <template
                    v-else-if="flowItem.groupType === flowTypeConstant.flow"
                  >
                    ...
                    <div class="flow-text">
                      <div class="text-line">
                        ...
                        <i
                          class="el-icon-close flow-delete"
                          v-if="
                            flowItem.type !== flowItemTypeConstant.startNode
                          "
                          @click.stop="handleDeleteFlowItem(flowItem)"
                        ></i>
                      </div>
                  ...
                  <template v-if="flowItem.groupType === flowTypeConstant.temp">
                    <draggable
                      class="flow-item node-temp node-temp-img"
                      ref="tempNode"
                      :id="flowItem.uuid"
                      :group="{ name: 'sortable', pull: false, put: true }"
                    >
      ...
      <div class="flow-editor-sidebar">
        <div v-for="(flowItem, index) in flowAllList" :key="index">
          <draggable
            class="items-box"
          ...
          >
            <template v-for="(item, index) in flowItem.children">
              ...
              <template
                v-else-if="flowItem.type === flowTypeConstant.condition"
              >
                <div
                  class="node-square"
                  :key="index"
      ...
      <template v-else>
        <div slot="title" class="dialog-title">
          <i
            ...
            v-if="dialogObj.editType !== flowItemTypeConstant.startNode"
            @click="handleGoList"
          ></i>
        ...
        <div class="dialog-body">
          <template v-if="dialogObj.editType">
            <component
              v-bind:is="dialogFlowItemComponent"
              ref="flowEditComponent"
              :editItem.sync="dialogObj.editForm"
            ></component>
          </template>
        </div>
      </template>
      <span slot="footer" class="dialog-footer">
        <el-button @click="handleCloseDialog" size="mini">Cancel</el-button>
        <el-button
          type="primary"
          @click="handleTempFlowFormData"
          size="mini"
          v-if="dialogObj.editType !== flowItemTypeConstant.list"
          >Confirm</el-button
        >
      </span>
    </el-dialog>

    <el-dialog title="JSON Data" :visible.sync="dialogVisible" width="700px">
...
export default {
  ...
  computed: {
    ...
    toggleSortClass() {
      return this.toggleAutoSort
        ? 'icon-auto-sort-open'
        : 'icon-auto-sort-close';
    },
  },
  methods: {
    // init flow
          ...
          flowList.forEach((item) => {
            item.next.forEach((id, index) => {
              ...
              if (this.isIfFlowItem(item.type)) {
                ...
                this.createFlowItemLabel(item.uuid, id, isIf ? 'Yes' : 'No');
    ...
    createFlowItemLabel(source, target, label) {
      this.$nextTick(() => {
        ...
        if (lines.length > 0) {
          lines[0]
            .getOverlay('flowItemDesc')
            .setLabel(
              `<span class="node-item-title" title="${label}">${label}</span>`
            );
        }
      });
    },
    ...
    deleteFlowItem(flowItem) {
      ...
      // delete item is has more flow item like ifElse or expand flow item
        ...
        if (this.isIfFlowItem(preFlowItem.type)) {
          this.addIfOneTempFlowItem(
            preFlowItem.uuid,
            preFlowItem.nextIfId === flowItem.uuid
          );
        }
        // pre is expand expand
        else if (this.isExpandFlowItem(preFlowItem.type)) {
          // this.addTempFlowItem(flowItem.prev[0]);
          const expandFlowItem = this.getExpandFlowItem(
            preFlowItem,
            flowItem.uuid
          );
          this.addExpandOneTempFlowItem(
            preFlowItem,
            flowItem.left,
            expandFlowItem.name
          );
    ...
    handleDeleteOnePrevOneNextFlowItem(flowItem, idx) {
      ...
      this.flowList.forEach((item) => {
        if (item.prev.length > 0 && item.prev.indexOf(flowItem.uuid) !== -1) {
          ...
          item.next.length > 0 &&
          item.next.indexOf(flowItem.uuid) !== -1
        ) {
      ...
      let expandFlowItem = {};
      if (
        flowItem.type === FLOW_ITEM_TYPE.endNode &&
        prevFlowList[0].type === FLOW_ITEM_TYPE.expandNode
      ) {
      ...
      this.$nextTick(function () {
        ...
        if (preFlowItem && nextFlowItem) {
          ...
          if (prevFlowListNextIndex !== -1) {
            preFlowItem.next.splice(
              prevFlowListNextIndex,
              0,
              nextFlowItem.uuid
            );
          } else {
          ...
          if (nextFlowListPrevIndex !== -1) {
            nextFlowItem.prev.splice(
              nextFlowListPrevIndex,
              0,
              preFlowItem.uuid
            );
          } else {
          ...
          if (this.isIfFlowItem(preFlowItem.type)) {
            ...
            let ruleGroupItem = this.getExpandFlowItem(
              preFlowItem,
              deleteFlowItem.uuid
            );
            if (ruleGroupItem) {
        ...
        if (flowItem.type === FLOW_ITEM_TYPE.endNode) {
          if (preFlowItem) {
            ...
            if (this.isIfFlowItem(preFlowItem.type)) {
              ...
              // create one flow item
              let tempFlowUuid = this.addExpandOneTempFlowItem(
                preFlowItem,
                flowItem.left,
                expandFlowItem.name
              );
    ...
    handleDeleteFlowItem(flowItem) {
      this.$confirm(
        'Are you sure you want to delete? Tips: ifNode and expandNode will delete all subsequent nodes',
        'Warning',
        {
    ...
    handleFlowMoveEnd(evt) {
      ...
      if (this.movedFlowItem) {
        ...
        if (
          this.movedFlowItem.dataset.source &&
          this.movedFlowItem.dataset.target
        ) {
          obj.isBetween = true;
      ...
      if (this.isOnePreOneNext(newFlowItem)) {
        ...
        //
          ...
          if (this.isIfFlowItem(preFlowItem.type)) {
            ...
            const offset =
              (isEven ? centerIndex : centerIndex + 1) * FLOW_LEFT_STEP_LENGTH +
              FLOW_IF_OFFSET_LENGTH;
            if (preFlowItem.nextIfId === newFlowItem.uuid) {
          ...
          this.addExpandTempFlowItem(
            tempFlowId,
            newFlowItem.formData.ruleGroupList
          );
        }
      }
      return newFlowItem;
        ...
        if (this.isExpandFlowItem(flowItemType)) {
          ...
          if (addRuleGroupList.length > 0) {
            ...
            addRuleGroupList.forEach((ruleGroupItem, index) => {
              let left =
                leftPosition + (index + 1) * (FLOW_LEFT_STEP_LENGTH * 2);
              let tempFlowUuid = this.createFlowItem(
                FLOW_ITEM_TYPE.tempNode,
                newFlowItem.uuid,
                { left }
              );
    ...
    handleUpdateExpandFlowItem(flowItem) {
      ...
      this.$nextTick(() => {
        this.addExpandTempFlowItem(
          flowItem.uuid,
          flowItem.formData.ruleGroupList
        );
      });
    },
        ...
        this.$nextTick(() => {
          ...
          this.$nextTick(() => {
            ...
            if (isPreFlowExpandType) {
              this.updateExpandFlowItemRuleGroup(
                expandFormData.ruleGroupList,
                nextFlowId,
                tempFlowUuid
              );
    ...
    handleAddBetweenCommonFlowItem(opts) {
      ...
      this.$nextTick(() => {
        if (this.isExpandFlowItem(prevFlowItem.type)) {
          ...
          this.updateExpandFlowItemRuleGroup(
            prevFlowItem.formData.ruleGroupList,
            nextFlowId,
            flowItemUuid
          );
    ...
    addIfOneTempFlowItem(preUuid, isIf) {
      ...
      this.draggableFlowConnect(preUuid, tempFlowUuid);
      this.createFlowItemLabel(preUuid, tempFlowUuid, isIf ? 'Yes' : 'No');
      if (isIf) {
      ...
      flowList.forEach((flowItem) => {
        ...
        if (flowItem.next.length > 1) {
          ...
          if (this.isIfFlowItem(flowItem.type)) {
            ...
            step.stepJson = JSON.stringify({
              ifNodeTitle: flowItem.formData.ifNodeTitle,
            });
          } else if (this.isExpandFlowItem(flowItem.type)) {
            // expand flow item
        ...
        // if and expand
        if (
          !this.isIfFlowItem(flowItem.type) &&
          !this.isExpandFlowItem(flowItem.type)
        ) {
    ...
    handleClear() {
      this.$confirm(
        'Are you sure you want to clear the data and redo?',
        'Warning',
        {
          confirmButtonText: 'Confirm',
          cancelButtonText: 'Cancel',
    ...
    $_adjustChild() {
      ...
      for (let i = this.tempLayerMap.length - 1; i >= 0; i--) {
        ...
        flowList.forEach((flowItem, index) => {
          ...
          if (
            leftFlowItem &&
            flowItem.left - leftFlowItem.left < FLOW_LEFT_STEP_LENGTH_MAX
          ) {
            const parentFlowItem = this.$_findCommonParentNode(
              leftFlowItem,
              flowItem
            );
            const leftOffset =
              Math.abs(flowItem.left - leftFlowItem.left) +
              FLOW_LEFT_STEP_LENGTH_MAX;
    ...
    $_centerChild(preFlowItem) {
      ...
      if (preFlowItem.next && preFlowItem.next.length > 0) {
        ...
        if (preFlowItem.next.length === 1) {
          ...
          const nextFlowItemLatest = this.getFlow(
            preFlowItem.next[preFlowItem.next.length - 1]
          );

          leftOffset =
            preFlowItem.left -
            (nextFlowItemFirst.left +
              (nextFlowItemLatest.left - nextFlowItemFirst.left) / 2);
        }
    ...
    $_findCommonParentNode(flowItem1, flowItem2) {
      if (flowItem1.prev[0] === flowItem2.prev[0]) {
        ...
        return this.$_findCommonParentNode(
          flowItem1PreFlowItem,
          flowItem2PreFlowItem
        );
      }
    },
    ...
    $_layoutChild(preFlow, layer) {
      ...
      nextList.forEach((nextFlowUUid, index) => {
        ...
        if (flowItem) {
          ...
          const startLeft =
            preFlow.left -
            (FLOW_LEFT_STEP_LENGTH_MAX * (nextListLength - 1)) / 2;
    ...
    isOnePreOneNext(flowItem) {
      return (
        flowItem.groupType === FLOW_TYPE.action ||
    ...
    isHasMoreNextFlowItemByType(flowItemType) {
      return (
        this.isIfFlowItem(flowItemType) || this.isExpandFlowItem(flowItemType)
      );
    },
    ...
    isHasStepCountFlowItem(flowItem) {
      return (
        flowItem.groupType === FLOW_TYPE.action ||
        flowItem.type === FLOW_ITEM_TYPE.startNode
      );
    },
  },
  beforeDestroy() {
